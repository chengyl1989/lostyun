...
export function SearchInterface({ selectedSearchResult }: SearchInterfaceProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      console.log('Search with params:', { query, searchParams });

      // 构建搜索请求参数 - 总是使用SmartSearch模型，通过searchParams.useFullText控制是否使用全文搜索
      const searchRequest: any = {
        query,
        model: 'SmartSearch', // 固定使用SmartSearch模型
        searchParams: {
          count: searchParams.count || '10',
          offset: searchParams.offset || '0',
          useFullText: searchParams.useFullText || 'false' // 传递全文搜索参数
        }
      };
      ...
      if (searchParams.useFullText) {
        searchRequest.searchParams.useFullText = searchParams.useFullText;
      }
        ...
        <div className="space-y-3 sm:space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            ...
            {modelsLoading ? (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-1">
                {/* 搜索方式 */}
                <div className="bg-gray-100 dark:bg-gray-700 rounded-xl p-4 flex-1 flex items-center">
                  <Brain className="w-5 h-5 text-blue-500 mr-3" />
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">智能搜索</span>
                </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <motion.button
              ...
              disabled={!query.trim() || isSearching || !apiConfig?.searchEndpointId}
          ...
          <div className="bg-white/60 dark:bg-gray-700/60 backdrop-blur-sm rounded-2xl p-4 border border-gray-200/50 dark:border-gray-600/50">
            <div className="flex flex-wrap items-center gap-3">
              ...
              {/* 全文搜索开关 */}
              <div className="flex items-center gap-2">
                <label className="text-xs font-medium text-gray-600 dark:text-gray-400">全文搜索:</label>
                <select
                  value={searchParams.useFullText || 'false'}
                  onChange={(e) => updateSearchParams('useFullText', e.target.value)}
                  className="px-2 py-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-xs focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  disabled={isSearching}
                >
